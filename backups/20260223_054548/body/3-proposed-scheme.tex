...
\textbf{實驗可重複性與真實效能探討}：在此必須特別說明，本研究所呈現之所有客觀評估數據，皆是基於三組不同隨機亂數種子（Random Seeds）進行獨立完整訓練後所取之平均結果。我們並未仿效部分文獻中為追求單一最高指標 (SOTA) 而採取在測試集上頻繁挑選最佳檢查點（Cherry-picking Checkpoints）的極端做法。因此，本實驗忠實呈現的結果與原文字面宣稱數據可能存在落差。例如：SpeechPrompt v2 原始文獻聲稱其模型於 ASV19 LA 能達到 13.5\% 的 EER，而本研究平均僅取得 18.50\%（即使我們在測試階段頻繁抽取特定 Checkpoint，最高也僅能勉強推進至 15\% 左右）。我們推測此落差源於本研究採用更嚴謹的 bfloat16 統一精度、未針對特定驗證集「刷榜」，以及避免了選擇性保留極端極值的偏差。這些誠實的評估數據更能反映出模型在未經刻意挑選保護下的真實防禦下限。
