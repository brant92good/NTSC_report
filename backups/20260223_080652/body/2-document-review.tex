...
\section{深偽語音偵測技術與代表性模型}

隨著語音合成技術的迭代，偽造語音的逼真程度已達肉耳難辨之境。為了應對日益嚴峻的資安威脅，深偽語音偵測（Deepfake Speech Detection）技術也發展出了各式的防禦架構。本節將探討目前偵測技術的兩大主流發展範式：其一為傳統的「預訓練—微調（Pre-train, Fine-tune）」範式；其二則是新興的「提示學習（Prompting）」範式。我們將分別回顧這兩種範式下的代表性模型。

\subsection{「預訓練—微調」範式下之代表性模型}

在此範式下，普遍以透過在大規模資料配合自監督學習（Self-Supervised Learning, SSL）方式預訓練的模型作為前端特徵提取器，並串接為特定下游任務設計的後端網路，最終在整體上進行參數微調。此範式在各式下游任務上建立了極高的性能標竿，但伴隨著需要龐大存儲空間以及算力資源的短板。以下將詳細介紹兩個在防偽偵測上極具代表性的架構：基於圖神經網路的 AASIST，以及基於 Conformer 架構與多尺度特徵融合的 MFA-Conformer。

\subsubsection{SSL-AASIST 模型}
...
\subsubsection{MFA-Conformer (Conformer with Transfer Learning)}
...
\subsection{基於提示學習之防禦範式 (Prompting Paradigm)}
...
\subsubsection{GSLM (Generative Spoken Language Model)}
...
\subsection{文獻總結與本計畫之研究定位}

綜合上述對語音合成與深偽偵測發展軌跡的回顧，我們可以觀察到一項在防禦端亟待解決的矛盾點：\textbf{傳統的 SOTA 防禦模型（如 SSL-AASIST）雖具備強大的偵測性能與跨域泛化力，但其極度昂貴的計算代價與龐大的參數量（\textasciitilde 318M），在實際落地部署時面臨巨大的阻礙}。為了因應輕量化與高效部署的需求，本研究挑選了三種具備不同架構哲學與參數規模的指標性模型進行深入的橫向對比與消融分析，藉此釐清防偽偵測的關鍵瓶頸。我們的研究定位與對比策略如下表 \ref{tab:model_comparison} 所示：

\begin{table}[H]
\centering
\caption{本研究選用之三種代表性偵測模型之核心特性比較}
\label{tab:model_comparison}
\begin{tabular}{llcc}
\toprule
\textbf{模型架構} & \textbf{訓練範式} & \textbf{特徵型態} & \textbf{可訓練參數量} \\
\midrule
\textbf{SSL-AASIST} & Pre-train, Full Fine-tune & 連續 (Continuous) & \textasciitilde 318M \\
\textbf{MFA-Conformer} & Pre-train, Partial Fine-tune & 連續 (Continuous) & \textasciitilde 14M \\
\textbf{SpeechPrompt v2} & Prompting Paradigm & 離散 (Discrete) & \textasciitilde 0.13M \\
\bottomrule
\end{tabular}
\end{table}

\begin{enumerate}
    \item \textbf{SSL-AASIST 作為效能指標天花板}：作為目前公認的 SOTA 標竿，其採用全參數微調的巨型模型策略。本研究將其視為「在不考慮計算資源下，基礎模型於偽造偵測任務所能達到的泛化能力上限」，並以此作為衡量其他輕量化架構的基準線。
    \item \textbf{MFA-Conformer 驗證中型架構的潛力}：作為中等規模（14M）的模型代表，其僅需 SSL-AASIST 不到 5\% 的參數量，卻能在語者辨識（ASV）等任務上取得優異表現。本研究將探討這種依賴局部-全域特徵（CNN+Transformer）的設計，在面對未知編碼破壞（如 ASV21 DF）時，其特徵聚合機制是否足以跨越領域鴻溝，或是受限於模型容量。
    \item \textbf{SpeechPrompt 探索極致參數效率的邊界與侷限}：作為僅需訓練 0.13M 參數（僅佔前者 0.04\%）的前衛架構，其在許多語意理解任務上已能比肩甚至超越 SOTA，但在防偽偵測（如 ASV19 LA）上卻存在顯著的性能落差（EER 13.5\% 對比 0.21\%）。
\end{enumerate}

\textbf{本計畫之核心目標}：本研究的定位並非單純比較哪一個模型擁有最低的錯誤率，而是要\textbf{深入探討導致效能落差的底層原因}。特別是針對 SpeechPrompt 展現出的巨大效能斷層，本計畫將透過一系列消融實驗（Ablation Studies），釐清其瓶頸究竟是來自於「Prompt Tuning 機制本身的學習能力不足」，還是「前端 GSLM 離散化過程抹除了鑑別真偽所需的連續聲學微小瑕疵」。唯有釐清導致泛化失敗與效能天花板的根本機制，未來才能在「極致參數效率（Prompting）」與「穩健泛化性能」之間，找出一條兩全其美的防禦架構演進指引。
