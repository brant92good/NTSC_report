...
\textbf{次要實驗 3：預訓練前端模型規模與訓練動態之影響}。為了探究前端特徵提取器的模型容量是否限制了整體的深偽偵測效能，我們選用不同規模的 NeMo 前端模型（TNASP Small、Medium、Large）進行了 50 個 Epoch 的完整訓練，並在每個 Epoch 結束時直接於各個測試集上進行軌跡追蹤（此驗證方式雖不符嚴格盲測規範，但有助於釐清學習動態與過擬合現象）。各模型在訓練過程中的 EER 動態變化如圖~\ref{fig:tnasp_epochs} 所示。

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{images/tnasp_epochs.png}
    \caption{不同規模 NeMo 前端（TNASP Small/Medium/Large）與原模型在訓練過程中的 EER 變化軌跡。}
    \label{fig:tnasp_epochs}
\end{figure}

從此學習動態中可以觀察到幾個關鍵現象：(1) \textbf{基礎任務的收斂與異常}：在標準的 ASV19 任務（Dev 與 Eval）中，多數模型都能迅速收斂並保持極低的錯誤率。但值得注意的是，TNASP Medium 在經歷 20 個 Epoch 後出現了災難性的崩潰現象（EER 異常飆升至接近 1.0），顯示擴增參數量可能在特定配置下引發不穩定的學習狀態；(2) \textbf{跨域性能的飽和}：針對跨領域的 ASV21 LA 與 DF 任務，即便不同規模的模型在訓練初期的震盪幅度不一，但在 30 個 Epoch 開始進入收斂期後，原本被寄望能捕捉更複雜特徵的大型模型（Medium、Large）並未展現出預期的相對優勢。其最終穩定下來的 EER 與 Small 版本甚至 Original 基準相比並無實質進步（在難度最高的 ASV21 DF 上皆停滯在略高於 0.20 的水位）。這清楚地表明，單純擴增預訓練前端的模型參數量，並無法有效跨越語音深偽偵測遭遇未知攻擊時的根本性瓶頸。
