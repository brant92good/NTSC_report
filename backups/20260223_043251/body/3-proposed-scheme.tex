...
\toprule
\textbf{Model} & \textbf{Train Data \textbackslash\ Test on} & \textbf{ASV19 LA} & \textbf{ASV21 LA} & \textbf{ASV21 DF} & \textbf{ITW} & \textbf{MLAADv2 (FULL)} & \textbf{MLAADv8 (FULL)} \\
\midrule
...
& \cellcolor{yellow}MLAADv2 & \textcolor{red}{21.29\%} & \textcolor{red}{30.26\%} & \textcolor{red}{28.39\%} & \textcolor{red}{18.77\%} & \cellcolor{gray!30}- & \cellcolor{gray!30}- \\
& \cellcolor{yellow}MLAADv8 & \textcolor{red}{26.85\%} & \textcolor{red}{30.61\%} & \textcolor{red}{23.48\%} & \textcolor{red}{16.03\%} & \cellcolor{gray!30}- & \cellcolor{gray!30}- \\
\midrule
\multirow{4}{*}{\shortstack[l]{SpeechPrompt v2\\(without DA)}} 
& ASV19 & \cellcolor{cyan!40}\textcolor{blue}{15.60\%} & 15.52\% & 24.90\% & 67.11\% & 99.65\% & 99.48\% \\
& ITW & \cellcolor{cyan!40}\textcolor{blue}{22.23\%} & \cellcolor{cyan!40}\textcolor{blue}{25.10\%} & \cellcolor{cyan!40}\textcolor{blue}{29.80\%} & \cellcolor{gray!30}- & \cellcolor{cyan!40}\textcolor{blue}{89.26\%} & \cellcolor{cyan!40}\textcolor{blue}{88.50\%} \\
& \cellcolor{yellow}MLAADv2 & 43.92\% & 44.68\% & 51.12\% & 63.31\% & \cellcolor{gray!30}- & \cellcolor{gray!30}- \\
& \cellcolor{yellow}MLAADv8 & \cellcolor{cyan!40}\textcolor{blue}{45.10\%} & \cellcolor{cyan!40}\textcolor{blue}{46.20\%} & \cellcolor{cyan!40}\textcolor{blue}{50.80\%} & \cellcolor{cyan!40}\textcolor{blue}{65.10\%} & \cellcolor{gray!30}- & \cellcolor{gray!30}- \\
\midrule
\multirow{4}{*}{\shortstack[l]{MFA-Conformer\\(without DA)}} 
& ASV19 & 2.13\% & 14.02\% & 16.00\% & 30.48\% & 92.61\% & 88.04\% \\
& ITW & 19.93\% & 26.48\% & 23.39\% & \cellcolor{gray!30}- & 37.31\% & 38.72\% \\
& \cellcolor{yellow}MLAADv2 & 34.90\% & 41.34\% & 38.14\% & 31.34\% & \cellcolor{gray!30}- & \cellcolor{gray!30}- \\
& \cellcolor{yellow}MLAADv8 & 45.27\% & 45.76\% & 42.98\% & 32.75\% & \cellcolor{gray!30}- & \cellcolor{gray!30}- \\
\bottomrule
\end{tabular}%
}
\end{table}

本節將對比不同模型架構在各測試集上的表現，實驗結果如表 \ref{tab:main_results} 所示。值得注意的是，為了釐清資料增強（Data Augmentation, DA）對不同架構的影響，我們僅對 \textbf{W2V-AASIST} 採用了其原始論文推薦的 \textbf{Rawboost} 增強策略，而其餘兩模型則未施加額外的資料增強。這是因為我們在預實驗中發現 \textbf{SpeechPrompt} 與 \textbf{MFA-Conformer} 在引入 DA 後並無顯著的性能提升，推測與輸入特徵的屬性（頻譜提取與離散化過程的濾波效應）有關。

\textbf{基準測試與訊號層級特徵的重要性}：在標準的 ASVspoof 2019 LA 與 2021 LA 測試中，基於 Wav2Vec 2.0 前端的 \textbf{W2V-AASIST} 展現了壓倒性的優勢，分別取得了 0.27\% 與 0.87\% 的極低 EER。這證實了在面對邏輯存取（Logical Access）攻擊時，模型對於原始波形（Raw Waveform）中細微頻譜特徵與相位資訊的捕捉能力至關重要。相比之下，\textbf{SpeechPrompt v2} 在 ASV19 LA 上的 EER 高達 15.60\%，與 SOTA 模型存在顯著差距。

\textbf{真實場景下的泛化能力}：在面對真實世界數據 \textbf{InTheWild (ITW)} 時，W2V-AASIST 雖然性能有所下降（13.30\% EER），但仍保持了一定的偵測能力，顯示出其對於通道變異具有一定的魯棒性。然而，SpeechPrompt 在 ITW 上表現出了極度的不適應，EER 高達 67.11\%，這基本上意味著模型在真實場景下完全失效。這一結果顯示缺乏底層聲學細節支撐的語意特徵並不足以應對深偽偵測任務。

\textbf{MLAAD 資料集的極端反差}：實驗中最引人注目的異常現象在於 MLAAD 資料集的結果。W2V-AASIST 在全語言版本（MLAAD FULL）上的準確率僅為 83.24\%，而 SpeechPrompt 卻在 MLAAD 上取得了驚人的 99.65\% 準確率。考慮到該模型在 ITW 與 ASV 基準上的低落表現，我們推測這並非源於模型真正的泛化能力，而是模型可能過度擬合（Overfitting）了 MLAAD 資料集生成過程中特定的、非一般化的特徵（如特定的靜音模式或 Codec 殘留）。

\textbf{MFA-Conformer 表現}：(實驗數據待補，此部分先留空，將展示 MFA-Conformer 在各測試集上的基礎性能，並與上述兩模型進行對比)。
...
\midrule
ASVspoof 19 LA & 15.60\% & 17.23\% \\
ASVspoof 21 LA & 15.52\% & 17.60\% \\
ASVspoof 21 DF & 24.90\% & 29.90\% \\
\bottomrule
\end{tabular}
\end{table}

實驗結果出乎意料，全參數量微調 (Full Fine-Tuning) 非但沒有提升模型表現，在 ASV 系列測試集上的等錯誤率 (EER) 反而略遜於僅使用 Prompt Tuning 的設定。這項結果帶來了兩個關鍵洞察：(1) \textbf{Prompting 的參數效率與正則化優勢}：僅訓練極少量的 Prompt 參數不僅已能激發預訓練語言模型的任務分類能力，甚至因避免了模型在有限資料上過度擬合 (Overfitting) 而擁有更好的泛化表現；(2) \textbf{瓶頸在於前端特徵}：即使開放所有基礎參數進行微調也無法突破效能天花板，暗示了問題的根源並非後端語言模型的學習能力，而是前端 S2u (Speech-to-unit) 階段的離散化過程造成了深偽偵測所需的不可逆聲學特徵流失。
...
\textbf{次要實驗 3：MFA-Conformer 預訓練模型規模之影響}。MFA-Conformer (Small) 在 ASV21 DeepFake (DF) 任務上與 W2V-AASIST 存在顯著落差（EER 15.00\% vs 5.57\%）。為了假設這是否源於模型容量不足，我們將前端替換為更大型的 \texttt{Conformer-Medium} 與 \texttt{Conformer-Large}。實驗結果顯示，即便將模型規模大幅提升，在 DF 測試集上的 EER 並未出現顯著下降，性能呈現飽和狀態。這暗示了單純增加模型參數並無法解決該架構在跨域偵測上的瓶頸。

\textbf{次要實驗 4：MFA-Conformer 後端分類器之改良}。我們推測 MFA 原始架構中簡單的統計池化與線性分類器可能無法完全整合 Conformer 抽取的複雜特徵，因此嘗試將 MFA-Conformer 中各層的特徵 (All layer) 導入並替換為 AASIST 後端架構。實驗數據如表~\ref{tab:exp4_mfaconformer} 與原始基礎對齊呈現。
...
\midrule
Linear Classifier (Baseline) & 2.13\% & 14.02\% & 16.00\% \\
AASIST Architecture (using All layer) & 0.86\% & 3.94\% & 12.71\% \\
\bottomrule
\end{tabular}
\end{table}

結果顯示這項改良帶來了顯著的性能提升：ASV19 LA 的 EER 從 2.13\% 大幅下降至 0.86\%；ASV21 LA 亦由 14.02\% 改善至 3.94\%；在處理難度最高的 ASV21 DF 資料集時，表現進步到 12.71\%。這證明原先簡易的分類網絡限制了 MFA-Conformer 的潛能，透過引入圖神經網路等更強大的特徵融合機制（如 AASIST），確實能更有效率地聚合多層次的前端特徵，並進一步增強防偽系統的穩健性。